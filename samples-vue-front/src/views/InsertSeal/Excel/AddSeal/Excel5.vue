	...
	const axios=require('axios');
	  export default{
	    ...
	    created: function(){
	      ...
	      axios.post("/api/InsertSeal/Excel/AddSeal/Excel5").then((response) => {
			    ...
			    DeleteAllSeal() {
			        ...
			        if (iCount > 0) {
			            for (var i = iCount - 1; i >= 0; i--) {
			                var strTempSealName = document.getElementById("PageOfficeCtrl1").ZoomSeal.Item(i).SealName;//获取加盖的印章名称
			                document.getElementById("PageOfficeCtrl1").ZoomSeal.Item(i).DeleteSeal();//删除印章
			            }
			        } else {
			            alert("请先在文档中加盖印章后，再执行当前操作。");
