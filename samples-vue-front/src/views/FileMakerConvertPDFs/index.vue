	...
	<div class="PDF">
		...
		<div id="div2" v-if="div2" style="margin:100px" align="center">
		    <form id="form1">
		        ...
		        <input type="button" value="批量转换成PDF文档" @click="getID()"/>
		    </form>
			...
			<div id="aDiv" style="display: none; color: Red; font-size: 15px;text-align:center;">
			    <span>转换完成，可在下面的地址中查看扩展名为“.pdf”文件，具体的地址为: {{url}}></span></span>
			</div>
	...
	const axios=require('axios');
	  export default{
	    ...
	    created: function(){
	      ...
	      axios.post("/api/FileMakerConvertPDFs/index").then((response) => {
	    ...
	    methods:{
			editWord(id){
				...
				//删除当前弹框页面其他div
				this.div2=false;
				axios.post("/api/FileMakerConvertPDFs/Word?id="+id).then((response) => {
		  ...
		  getID() {
			  ...
			  //第一次自刷新
			  document.getElementById("iframe1").src = axios.post("/api/FileMakerConvertPDFs/Convert?id="+this.str[this.count]).then((response) => {
														this.poHtmlCode = response.data;
