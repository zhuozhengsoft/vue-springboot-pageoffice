...
public class WordResExcelController {
    ...
    public String showWord(HttpServletRequest request) {
        ...
        data1.setValue("[excel]/api/doc/WordResExcel/1.xls[/excel]");
        DataRegion data2 = worddoc.openDataRegion("PO_p2");
        data2.setValue("[word]/api/doc/WordResExcel/2.doc[/word]");
        DataRegion data3 = worddoc.openDataRegion("PO_p3");
        data3.setValue("[word]/api/doc/WordResExcel/3.doc[/word]");

        poCtrl.setWriter(worddoc);
        poCtrl.setCaption("演示：后台编程插入excel文件到数据区域(企业版)");
        //隐藏菜单栏
        poCtrl.setMenubar(false);
        //隐藏自定义工具栏
        poCtrl.setCustomToolbar(false);
        //打开Word文档
        poCtrl.webOpen("/api/doc/WordResExcel/test.doc", OpenModeType.docNormalEdit, "张三");
